...
window.addEventListener('load', () => {
  ...
  document.addEventListener('click', event => {
    if (event.target.id == 'update-button') {
      ...
      serverRequest(`/book/writing/edit?id=${book._id}&writing_id=${writing._id}`, 'POST', {
        title,
        writer_id: writerId,
        subtitle,
    ...
    if (event.target.id == 'update-turkish-button') {
      ...
      serverRequest(`/book/writing/translate?id=${book._id}&writing_id=${writing._id}`, 'POST', {
        language: 'tr',
        title,
        subtitle,
        flag,
  ...
  document.addEventListener('change', event => {
    if (event.target.id == 'image') {
      if (ancestorWithClassName(event.target, 'turkish-translation') || ancestorWithClassName(event.target, 'russian-translation')) {
        ...
        serverRequest(`/book/writing/logo-translate?id=${book._id}&writing_id=${writing._id}&language=${language}`, 'FILE', {
          file
        }, res => {
        ...
        serverRequest(`/book/writing/logo?id=${book._id}&writing_id=${writing._id}`, 'FILE', {
          file
        }, res => {
          if (!res.success) return throwError(res.error);
