...
window.addEventListener('load', () => {
  const parent = JSON.parse(document.getElementById('parent-json').value);
  ...
  document.addEventListener('click', event => {
    if (event.target.id == 'update-button') {
      ...
      serverRequest(`/book/writing/edit?${parent_type ? 'type=' + parent_type + '&' : ''}id=${parent._id}&writing_id=${writing._id}`, 'POST', {
        title,
        writer_id: writerId,
        subtitle,
    ...
    if (event.target.id == 'update-turkish-button') {
      ...
      serverRequest(`/book/writing/translate?${parent_type ? 'type=' + parent_type + '&' : ''}id=${parent._id}&writing_id=${writing._id}`, 'POST', {
        language: 'tr',
        title,
        subtitle,
        flag,
  ...
  document.addEventListener('change', event => {
    if (event.target.id == 'image') {
      if (ancestorWithClassName(event.target, 'turkish-translation') || ancestorWithClassName(event.target, 'russian-translation')) {
        ...
        serverRequest(`/book/writing/logo-translate?${parent_type ? 'type=' + parent_type + '&' : ''}id=${parent._id}&writing_id=${writing._id}&language=${language}`, 'FILE', {
          file
        }, res => {
        ...
        serverRequest(`/book/writing/logo?${parent_type ? 'type=' + parent_type + '&' : ''}id=${parent._id}&writing_id=${writing._id}`, 'FILE', {
          file
        }, res => {
          if (!res.success) return throwError(res.error);
