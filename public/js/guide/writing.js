...
function saveContent(callback) {
  ...
  if (!translate || translate == 'en') {
    serverRequest(
      `/guide/writing/edit?id=${guide._id}&writing_id=${writing._id}`,
      'POST',
      generateWritingData(),
      res => {
        if (!res.success) {
    ...
    serverRequest(
      `/guide/writing/translate?id=${guide._id}&writing_id=${writing._id}`,
      'POST',
      data,
      res => {
        if (!res.success) {
...
window.addEventListener('load', () => {
  guide = JSON.parse(document.getElementById('guide-json').value);
  ...
  coverInput.addEventListener('change', event => {
    ...
    if (!translate || translate == 'en') {
      serverRequest(`/guide/writing/cover?id=${guide._id}&writing_id=${writing._id}`, 'FILE', {
        file
      }, res => {
        if (!res.success) {
      ...
      serverRequest(`/guide/writing/cover-translate?id=${guide._id}&writing_id=${writing._id}&language=${translate}`, 'FILE', {
        file
      }, res => {
        if (!res.success) {
          imageLoadingPrompt.style.display = 'none';
  ...
  document.addEventListener('click', event => {
    if (ancestorWithClassName(event.target, 'change-language-input-choice')) {
      ...
      saveContent(err => {
        ...
        window.location = `/guide/writing/content?id=${guide._id}&writing_id=${writing._id}&translate=${translate}`
      });
    };
